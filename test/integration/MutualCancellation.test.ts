...
let mutualCancellationEscrowCounter = 0n;
...
async function setupMutualCancellationContracts() {
  ...
  mutualCancellationEscrowCounter = 0n;
...
async function createMutualCancellationTestEscrow(
  ...
  const agreementParams: EscrowAgreement = {
    ...
    nonce: mutualCancellationEscrowCounter, // Use counter as nonce
  ...
  const escrowId = mutualCancellationEscrowCounter;
  mutualCancellationEscrowCounter++; // Increment for next escrow
...
describe('Comprehensive Mutual Cancellation Tests', () => {
  ...
  test('🔄 MUTUAL CANCELLATION: Provider Initiates with Holder Signature', async () => {
    ...
    // Step 4: Provider calls mutualCancel with holder's signature
    ...
    try {
      ...
      console.log(`✅ Mutual cancellation successful! TX: ${mutualCancelTxHash.slice(0, 20)}...`);
  ...
  test('🚫 MUTUAL CANCELLATION: Invalid Signature Rejection', async () => {
    ...
    try {
      await escrowContract.write.mutualCancel(
  ...
  test('⚠️ MUTUAL CANCELLATION: State Validation', async () => {
    ...
    await escrowContract.write.holderCancel([closedEscrowId], { account: holder.account });
  ...
  test('💰 MUTUAL CANCELLATION: Fund Distribution Verification', async () => {
    ...
    try {
      const cancelTxHash = await escrowContract.write.holderCancel([escrowId], { account: holder.account });
